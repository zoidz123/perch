...
async function attachNativeTui(session) {
  ...
  // Codex hands every dynamic tool call to EVERY client attached to the thread
  // and keeps the FIRST answer. The native TUI is a full app-server client that
  // answers them with "Dynamic tool calls are not available in TUI yet", so
  // while it is attached this session's Perch tools (report_task_event,
  // send_report, autoreview_run, delivery_create_pr) race the TUI and mostly
  // lose, surfacing to the agent as "dynamic tool request failed". Detaching
  // restores them. Verified live against codex-cli 0.146.0. Claude sessions
  // mirror the Perch-owned PTY instead and are unaffected.
  console.error(
    "Warning: attaching suspends this worker's custom tool calls (reporting, review, delivery) " +
      "until you detach - Codex TUI limitation."
  );
